...
async function main() {
  console.log('\n🔄 Resetting and re-inserting policies...\n');

  // Delete old policies for this agent (keep them for now, just test with new ones)
  console.log('📋 Step 1: Deleting old policies for this agent...');
  ...
  if (deleteError) {
    console.error('❌ Error deleting old policies:', deleteError);
    // Continue anyway
  } else {
    console.log('✅ Deleted old policies');
  }

  console.log('\n📋 Step 2: Inserting 26 new policies...\n');
  ...
  console.log(`✅ Verification: Found ${verify?.length || 0} policies for this agent+client`);
